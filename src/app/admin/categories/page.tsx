...
} from "@/components/shared/category/useCategory";
...
} from "@/components/shared/category/CategoryForm";
...
export default function CategoryListPage() {
  ...
  const [parentIdFilter, setParentIdFilter] = React.useState<string>(
  ...
  const [showDeleted, setShowDeleted] = React.useState<boolean>(
  ...
  const filter = React.useMemo<CategoryFilter>(
    () => ({
      page,
      limit,
    ...
    [page, limit, nameFilter, parentIdFilter, isSubCategory, showDeleted]
  );
  ...
  const [createDefaultParentId, setCreateDefaultParentId] = React.useState<
    string | undefined
  >(undefined);
  const [createForm, setCreateForm] = React.useState<CategoryFormValue>({
    name: "",
    notes: "",
    parentId: "",
  });
  const [editForm, setEditForm] = React.useState<CategoryFormValue>({
    name: "",
    notes: "",
    parentId: "",
  });
  ...
  React.useEffect(() => {
    ...
    if (current !== href) {
      const nextParams = {
        ...
        name: nameFilter || undefined,
        parentId: parentIdFilter || undefined,
        isSubCategory: isSubCategory === true ? true : undefined,
        deleted: showDeleted || undefined,
      };
  ...
  return (
    <div className="relative p-4 md:p-6 ">
      <div className="mb-4">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <CategoryFilters
            ...
            onSearchSubmit={(v) => {
              setNameFilter(v !== undefined ? v : nameInput);
              setPage(1);
            }}
          />
      ...
      <AdminActionDialog
        ...
        description={
          <span>
            Bạn có chắc muốn xoá danh mục "<b>{target?.name}</b>"?
          </span>
        }
          ...
          createCategory(
            {
              ...
              notes:
                createForm.notes !== undefined
                  ? normalizeSpaces(createForm.notes)
                  : undefined,
            ...
            },
            {
              ...
              onError: (e: any) =>
                notify({
                  title: "Lỗi",
                  description: String(e?.message || "Không thể tạo"),
                  variant: "destructive",
                }),
            }
          );
      ...
      >
        <CategoryForm
          ...
          onValidate={(field, v) => {
            if (field === "name") {
              setNameTouched(true);
              setNameError(validateCategoryName(v));
            }
          }}
          parents={(parentsData?.data ?? []) as any}
          ...
          updateCategory(
            {
              ...
              notes:
                editForm.notes !== undefined
                  ? normalizeSpaces(editForm.notes)
                  : undefined,
