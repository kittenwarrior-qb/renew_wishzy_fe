...
export default function Page() {
  ...
  const updateURL = React.useCallback(
    (newPage: number, newLimit: number, newStatus: string) => {
      ...
      router.replace(`/instructor/quiz-attempts?${params.toString()}`, {
        scroll: false,
      });
    },
    [router]
  ...
  const columns: Column<QuizAttemptListItem>[] = [
    {
      key: "user",
      title: "Học viên",
      render: (row: QuizAttemptListItem) => (
    ...
    },
    {
      ...
      title: "Trạng thái",
      align: "center",
      render: (row: QuizAttemptListItem) => getStatusBadge(row.status),
    },
    {
      key: "startedAt",
      title: "Bắt đầu",
  ...
  return (
    <div className="relative p-4 md:p-6">
      ...
      {/* Page Header */}
      <div className="mb-6">
        <h1 className="text-2xl font-bold">Bài làm của học viên</h1>
        <p className="text-muted-foreground">Xem các bài làm của học viên cho bài kiểm tra của bạn</p>
      </div>
