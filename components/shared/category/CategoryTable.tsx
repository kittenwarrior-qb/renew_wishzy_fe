...
function ChildrenBranch({
  ...
  if (isError || items.length === 0)
    return (
      <div className="text-xs text-muted-foreground">Không có danh mục con</div>
    );
  return (
              ...
              return (
                <React.Fragment key={String(item.id)}>
                  <tr className="border-t hover:bg-muted/30">
                    <td className="p-3 w-16 text-muted-foreground text-center">
                      {stt}
                    </td>
                      ...
                      >
                        ...
                        <ChevronDown
                          className={`h-4 w-4 text-muted-foreground transition-transform ${
                            expanded.has(String(item.id)) ? "rotate-180" : ""
                          }`}
                        />
                      </button>
                    </td>
                    ...
                    <td className="p-3 text-center">
                      <div className="flex justify-center gap-1">
                        ...
                        <Button
                          ...
                          size="icon"
                          className="cursor-pointer"
                          onClick={() => onDelete(String(item.id), item.name)}
                          title="Xoá"
                          disabled={deleting}
                        >
                    ...
                    <tr className="border-t bg-muted/20">
                      <td colSpan={5} className="p-3">
                        <InlineChildrenRow
                          ...
                          onDeleteChild={(cat) =>
                            onDelete(String(cat.id), cat.name)
                          }
                          onViewChild={(cat: Category) => {
                            setViewingId(String(cat.id));
                            setOpenView(true);
                          }}
                        />
                      </td>
          ...
          {viewing ? (
            <div className="space-y-6">
                  ...
                  {(viewing as any)?.parentId ? (
                    <div className="mt-2 inline-flex items-center gap-2">
                      <span className="rounded-full border px-2 py-0.5 text-[11px] text-muted-foreground">
                        Danh mục cha:{" "}
                        {parentNameById.get(
                          String((viewing as any).parentId)
                        ) || `(ID: ${String((viewing as any).parentId)})`}
                      </span>
                    </div>
                  ) : null}
                </div>
              ...
              <div>
                ...
                <div className="rounded-md border p-3">
                  <ChildrenBranch
                    parentId={String((viewing as any)?.id || viewingId || "")}
                  />
                </div>
              </div>
